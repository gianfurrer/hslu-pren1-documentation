...
\subsection{Morphologischer Kasten}
    ...
    Die Fortbewegung und Lenkung wird mit dem Prinzip ''Roomba'' realisiert, wobei die beiden angetriebenen Räder mittels je eines DC-Motors angetrieben werden. Dies ermöglicht eine einfache Ansteuerung und Drehung auf der Stelle. Die Hindernisse werden von der Seite geklemmt und aufgenommen, der Antrieb dazu wird mit einem Linearmotor realisiert. Mit dem aufgenommenen Hindernis dreht sich das Fahrzeug um die eigene Achse und setzt dieses wieder ab. Mittels Encoder wird die Position der DC-Motoren überprüft. Es werden Encoder gewählt, aufgrund ihrer einfachen Funktionsweise und Auslösbarkeit. Die Software läuft auf einem Raspberry Pi und die Hardwaresteuerung auf einem TinyK22. Auf diesen Plattformen haben unsere Informatiker wie auch Elektrotechniker schon Erfahrungen gesammelt. Die Hindernisse sollen mit einem Ultraschall-Sensor erkannt werden. Um eine Redundanz zu erhalten, wird mittels Infrarotsensor nachkontrolliert, ob wirklich ein Hindernis im Weg steht. Diese Reduanz wird eingesetzt, da die Sensoren nicht immer den genauen Wert messen. Die Pylonen werden mit der Kamera, welche auch zur allgemeinen Streckenerkennung verwendet wird, erkannt. Mittels Infrarot-Sensor soll auch für die Pylonen ein Backup-Signal generiert werden. Mit einem Liniensensor wird überprüft, ob das Fahrzeug sich auch wirklich entlang des Klebestreifens bewegt. Um eine Linie zuerkennen ist der Liniensensor die einfachste Lösung. Dasselbe gilt für die Verifizierung von Wegpunkten, welche primär mit einem Farbsensor erkannt werden. Die Software für die Objekterkennung ist in YOLO, während die Wegfindung auf Dijkstra läuft. Für die Energieversorgung wird ein Lithium-Polymer-Akku eingesetzt. Da dieser die grössten Vorteile hat siehe \ref{subsec:Energiequelle}.

    \paragraph{Lösungsvariante Beweglich (gelb)}
    Die Fortbewegung und Lenkung wird mit Mecanum-Rädern realisiert, wobei die vier Räder mittels je einem Schrittmotor angetrieben werden. Somit kann sich das Fahrzeug in alle vier Richtungen linear bewegen. Die Hindernisse werden von vorne und hinten geklemmt und aufgenommen, der Antrieb dazu wird mit einem Linearmotor realisiert. Mit dem aufgenommenen Hindernis dreht sich das Fahrzeug um die eigene Achse und setzt dieses wieder ab. Mittels Beschleunigungssensor wird die Position des des Fahrzeuges überprüft. Die Software läuft auf einem Raspberry Pi und die Hardwaresteuerung auf einem Arduino. Die Hindernisse sollen mit einem Infrarotsensor erkannt werden. Um eine Redundanz zu erhalten, wird mittels Ultraschallsensor nachkontrolliert, ob wirklich ein Hindernis im Weg steht. Die Pylonen sollen mit einem Infrarotsensor erkannt werden und mittels Ultraschall bestätigt werden. Die Fahrstrecke wird mit einem Liniensensor erkannt und mit der Kamera kontrolliert. Wegpunkte werden ebenfalls mit Liniensensoren gesehen und mit einem Farbsensor bestätigt. Die Software für die Objekterkennung ist ein YOLO, während die Wegfindung auf A* läuft. Für die Energieversorgung wird ein Lithium-Ionen-Akku eingesetzt.

    \paragraph{Entscheid Lösungsvariante}
    Umgesetzt wird die Lösungsvariante A (grün). Aufgrund von Ihrer Einfachheit und Robustikeit. Es wird höher gewertet sicher in das Ziel zukommen als grosse Risiken einzugehen. 
...
\footnotetext{Quelle:\href{https://www.rcteam.com/de/blogs/der-blog-des-rc-modellbaus/nimh-vs-lipo-batterien}{www.rcteam.com}}
